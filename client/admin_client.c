...
static void list_patients(void) {
    ...
    print_separator();
    if (cnt == 0) printf(DIM "  (no patients)\n" RESET);
    /* Read until the sentinel '.' regardless of cnt — the count is just
     * a hint for display; the dot is the true end-of-stream marker. */
    while (recv_line(srv_fd, buf, sizeof(buf)) > 0) {
        if (strcmp(buf, ".") == 0) break;   /* sentinel always consumed */
        int pid, age, doc; char name[64], cond[128];
        sscanf(buf, "%d|%63[^|]|%d|%127[^|]|%d", &pid, name, &age, cond, &doc);
        printf("  %-5d  %s%-25s%s  %-5d  %-25s  %d\n",
               pid, BRIGHT_CYAN, name, RESET, age, cond, doc);
    }
}
...
static void assign_nurse_menu(void) {
    ...
    recv_line(srv_fd, buf, sizeof(buf));
    if (strncmp(buf,"OK",2)==0) printf(GREEN "  ✓ Nurse assigned\n" RESET);
    else printf(RED "  ✗ %s\n" RESET, buf);
}
...
int main(void) {
    ...
    if (connect(srv_fd,(struct sockaddr*)&s,sizeof(s))<0){ perror("connect"); return 1; }
